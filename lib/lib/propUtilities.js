...
function assignPropType(propVal) {
  if (_lodash2.default.isArray(propVal)) {
    ...
    return _propTypes2.default.oneOfType(typeValues);
  }
...
function getUIClassString(props, state) {
  ...
  var orientation = state && state.orientation ? state.orientation : props.orientation;

  var sUIclass = "ui-" + uiclass;
  var hasUIclass = !className || className.indexOf(sUIclass) === -1;
  var coreClasses = (_coreClasses = {}, _coreClasses[sUIclass] = hasUIclass, _coreClasses["ui-anchor-" + anchor] = anchor, _coreClasses["ui-orientation-" + orientation] = orientation, _coreClasses["ui-content-align-" + contentAlign] = contentAlign, _coreClasses["ui-text-align-" + textAlign] = textAlign, _coreClasses["ui-overlay"] = overlay, _coreClasses["ui-fixed"] = fixed, _coreClasses["ui-inset"] = inset, _coreClasses["ui-raised"] = raised, _coreClasses["ui-reveal"] = reveal, _coreClasses["ui-rounded"] = rounded, _coreClasses["ui-circular"] = circular, _coreClasses["ui-well"] = well, _coreClasses["ui-panel"] = panel, _coreClasses["ui-masked"] = masked, _coreClasses["ui-shadow"] = shadow, _coreClasses["ui-blurred"] = !!loader, _coreClasses["ui--collapsed"] = collapsed, _coreClasses["ui--open"] = open, _coreClasses["ui--closed"] = closed, _coreClasses["ui--hidden"] = hidden, _coreClasses["ui--shown"] = shown, _coreClasses.disabled = disabled, _coreClasses.active = active, _coreClasses.invalid = invalid, _coreClasses);
  ...
  if (!_lodash2.default.isNil(color)) {
    var nohover = String(color).indexOf("!") !== -1;
    var cleanColor = String(color).replace("!", "");
    ...
    if (nohover) {
      colorClass += "-no-hover";
    } else if (!_lodash2.default.isNil(colorHover)) {
      colorClass += "-hover";
    }
    styleClasses[colorClass] = true;
  }
  var iconPre = icon && icon.indexOf("ui-icon-") !== -1 ? icon : "ui-icon-" + icon;
